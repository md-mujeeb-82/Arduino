...
#define HOSTNAME "UPSSwitchOverTwin"
#define DEVICE_TITLE "UPS Automatic Switch Over - Twin"
...
const int RELAY_PIN_1 = 0;
const int AC_VOLTAGE_SENSE_PIN_1 = 2;
const int RELAY_PIN_2 = 4;
const int AC_VOLTAGE_SENSE_PIN_2 = 5;
...
void setup() {
  ...
  pinMode(AC_VOLTAGE_SENSE_PIN_1, INPUT_PULLUP);
  pinMode(AC_VOLTAGE_SENSE_PIN_2, INPUT_PULLUP);
...
void loop() {
  ...
  if (currentTime - lastTimeACRead > AC_SAMPLING_DELAY || lastTimeACRead == -1) {
      ...
      if(digitalRead(AC_VOLTAGE_SENSE_PIN_1) == LOW) {
         digitalWrite(RELAY_PIN_1, HIGH);
      } else {
         digitalWrite(RELAY_PIN_1, LOW);
      }

      if(digitalRead(AC_VOLTAGE_SENSE_PIN_2) == LOW) {
...
void handleRoot() {
  ...
  bool switchState = digitalRead(RELAY_PIN_1) == LOW;
