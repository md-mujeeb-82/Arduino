  ...
  if(currentTime - lastTimeButtonPress > TIMEOUT_RESET) {
    ...
    if(data.isAutoPilotMode) {
      if(isAutoPilotOn) {
        isAutoPilotOn = false;
        // Blink fast 5 times
        blinkLED(20, true, 5, true);
      } else {
        if(data.count + data.step > data.targetCount) {
          isAutoPilotOn = false;
          blinkLED(1000, true, 1, false);
        } else {
          isAutoPilotOn = true;
          // Blink fast 3 times
          blinkLED(20, true, 3, true);
        }
      }
...
void incrementTasbeeh() {
  ...
  if(data.count == data.targetCount) {
    isAutoPilotOn = false;
    tasbeehCompleteEffects();
  } else if(data.count % 100 == 0){
    blinkLED(10, true, 2, false);
  } else {
    blinkLED(10, true, 1, true);
  }
}

void blinkLED(int millisecs, bool withSound, int count, bool whichFrequency) {
  ...
  if(data.isBuzzerOn) {
    analogWrite(PIN_BUZZER, whichFrequency ? 75 : 150);
    isBuzzerOn1 = true;
  }
  digitalWrite(PIN_LED, HIGH);
  isVibratorOn1 = true;
...
void tasbeehCompleteEffects() {
  blinkLED(20, true, 3, true);
  blinkLED(1000, true, 1, true);
}
